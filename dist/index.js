...
function createMcpServer() {
    ...
    async function initPool(host, port, user, password) {
        if (pool) {
    ...
    function getPool() {
        if (!pool)
            throw new Error('Not connected. Call the "connect" tool first with your DB credentials.');
        return pool;
    }
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (name === 'connection_status') {
                if (!pool)
                    return { content: [{ type: 'text', text: 'Not connected. Call the "connect" tool with your host, user, and password.' }] };
